// PA 2.cpp : Defines the entry point for the console application.
//
...
int main(int argc, char **args) {
	...
	if (read_args(argc, args, &params)) {
		...
		if (myfile.is_open()) {
			...
			tic = std::clock();
			list.printConvexHull(params.latitude, params.longitude, params.radius, params.filter);
			toc = std::clock();
			std::cerr << "done!\n\t" << " records fetched in " << (double)(toc - tic) / CLOCKS_PER_SEC << " secs\n";
		}
	}
